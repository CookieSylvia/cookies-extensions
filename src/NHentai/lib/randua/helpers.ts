...
export const JSONIsFrequency = function(json: any): boolean {
    /* 
  ...
  */
    const [_, frequency] = Object.entries(json[Object.keys(json)[0] as any])[0] as any
  ...
  */
    ...
    for (const key in content) {
        ...
        for (const [userAgent, indexes] of Object.entries(content[key]!)) {
            (contentParsed[key] as any)[userAgent] = indexes.maxIndex - indexes.minIndex + 1
        }
    }
  ...
  */
    ...
    for (const key in content) {
        ...
        for (const [userAgent, frequency] of Object.entries(content[key]!)) {
            (contentParsed[key] as any)[userAgent] = sortedFrequencies.indexOf(frequency) + 1
        }
    }
    return contentParsed
}
...
export const JSONinterval = function(content: FrequencyJson): IntervalJson {
  ...
  */
    ...
    for (const key in content) {
        ...
        let minIndex = 0
        for (const [userAgent, frequency] of Object.entries(content[key]!)) {
            (contentParsed[key] as any)[userAgent] = {
                minIndex: minIndex,
