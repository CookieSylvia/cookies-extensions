...
export interface SearchResultsMany {
  history: SearchEntry[];
  partials: PartialSourceManga[];
  metadata: SearchMetadata;
}
...
export const Search = {
  ...
  searchMany: async (pages: number, ctx: SearchContext, objects: SearchObjects, metadata?: SearchMetadata): Promise<SearchResultsMany> => {
    ...
    for (let i = 0; i < pages; i++) {
      ...
      if (metadata.shouldStop || (results.partials ?? []).length <= 0) {
        break;
      }
    }
    return {
  ...
  searchApi: async (
    ...
    if (data.parsed != undefined) {
      ...
      return {
        partials: data.parsed.books.map((book) => BookParser.partial(book)),
        metadata: {
          nextPage: page + 1,
  ...
  searchFallback: async (
    ...
    if (data.parsed != undefined) {
      return {
        partials: data.parsed.map((booklet) => BookParser.partialFallback(booklet)),
        metadata: {
          nextPage: page + 1,
          maxPage: metadata?.maxPage,
          shouldStop,
  ...
  searchBookId: async (ctx: SearchContext, objects: SearchObjects): Promise<SearchResults> => {
    ...
    if (data.parsed != undefined) {
      return {
        partials: [BookParser.partial(data.parsed)],
        metadata: {
          shouldStop: true,
        },
        status: data.status,
